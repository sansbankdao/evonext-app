...
export function encodeAvatarFeatures(features: AvatarFeatures): string {
  ...
  const featureOrder: (keyof AvatarFeatures)[] = [
    'style', 'hair', 'hairColor', 'skinTone', 'eyes', 'eyeColor',
    'mouth', 'facialHair', 'accessories', 'clothing', 'clothingColor', 'background'
  ]
...
function getBackgroundColor(index: number): string {
  const colors = [
    ...
    'a8e6cf', 'dcedc1', 'ffeaa7', 'fab1a0', 'ff7675',
    '74b9ff', 'a29bfe', 'fd79a8', 'fdcb6e', '6c5ce7', 'e17055'
  ]
...
function getSkinColor(index: number): string {
  const colors = [
    'f8d25c', 'f0c687', 'daa15f', 'bf8f4f',
    'ae5d29', '935d37', '763900', '45260d'
  ]
  return colors[index % colors.length]
}
...
function getHairStyle(index: number): string {
  const styles = [
    ...
    'shavedSides', 'straight01', 'straight02', 'straightAndStrand', 'dreads01'
  ]
...
function getHairColor(index: number): string {
  const colors = [
    'a55728', '2c1b18', '272421', '3d2314', '5a3825',
    '7a4e48', 'b55239', 'd6b370', 'e8e1e1', 'ff7070',
    'ffc0cb', '8b4513', '4a4e4d', '0e0e0e', '6a4c93', '1e90ff'
  ]
  return colors[index % colors.length]
}
...
function getEyeStyle(index: number): string {
  const styles = [
    ...
    'wink', 'winkWacky', 'xDizzy', 'happyRounded', 'sadConcerned', 'unibrow'
  ]
  return styles[index % styles.length]
}
...
function getMouthStyle(index: number): string {
  const styles = [
    ...
    'twinkle', 'vomit', 'laughing', 'worried', 'smallSmile', 'frown'
  ]
  return styles[index % styles.length]
}
...
function getFacialHairStyle(index: number): string {
  const styles = [
    ...
    'moustacheFancy', 'moustacheMagnum', 'goatee', 'soul'
  ]
  return styles[index % styles.length]
}
...
function getAccessoryStyle(index: number): string {
  const styles = [
    ...
    'sunglasses', 'wayfarers', 'earbuds', 'winterHat01',
    'winterHat02', 'winterHat03', 'winterHat04', 'cap',
    'frizzle', 'headdress', 'hatCowboy'
  ]
  return styles[index % styles.length]
}
...
function getClothingStyle(index: number): string {
  const styles = [
    ...
    'shirtVNeck', 'sweater', 'uniform', 'dress', 'denimJacket',
    'leatherJacket', 'winterCoat', 'turtleneck'
  ]
  return styles[index % styles.length]
}
...
function getClothingColor(index: number): string {
  const colors = [
    '3c4f5c', '65c9ff', '5199e4', '25557c', '262e33',
    '929598', '110e0e', 'e6e6e6', 'ff5c5c', 'ff6b6b',
    '77311d', 'fc909f', 'f4d150', 'f5e050', '25557c', 'e1e1e1'
  ]
  return colors[index % colors.length]
}